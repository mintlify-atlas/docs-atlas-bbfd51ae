...
<Tip>
  To add a **Remember Me** feature, add a `RememberMe` boolean property to `LoginModel` and bind it to a checkbox in the form. Then pass it through to `AuthenticateUser.ExecuteAsync`. The `LoginRequest` DTO already includes a `RememberMe` parameter, so the use case and session manager are already wired to honour it — the cookie persistence duration is controlled inside `CookieSessionManager.SignInAsync()`.
</Tip>
